...
for _ in range(100):
    ...
    tight_deltas.append(
        abs(t["paid_search"]["mean"] - posterior["paid_search"]["mean"])
    )
    loose_deltas.append(
        abs(l["paid_search"]["mean"] - posterior["paid_search"]["mean"])
    )

assert np.mean(tight_deltas) < np.mean(
    loose_deltas
), "Higher epsilon should produce less noise"
print(
    f"Noise scale correct — tight eps avg delta: {np.mean(tight_deltas):.4f}, "
