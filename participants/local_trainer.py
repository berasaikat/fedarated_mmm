...
class LocalTrainer:
    def __init__(self, participant_id: str, config_path: str):
        ...
        relative_data_path = self.config.get(
            "data_path", f"data/synthetic/{participant_id}.csv"
        )
    ...
    def load_data(self):
        ...
        if not self.csv_path.exists():
            raise FileNotFoundError(
                f"Participant {self.participant_id} data not found at: {self.csv_path}"
            )
    ...
    def train(self, priors_dict: dict) -> dict:
        ...
        for param, stats in summary.items():
            if stats["r_hat"] > 1.05:
                print(
                    f"WARNING: {param} r_hat={stats['r_hat']:.3f} — chain may not have converged"
                )
