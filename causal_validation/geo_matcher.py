...
class GeoMatcher:
    ...
    def match(
        self, treated_geo_description: str, candidate_geos_list: List[Dict[str, str]]
    ) -> List[str]:
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "ranked_geo_ids" not in parsed_data:
                    raise ValueError(
                        "Missing structured 'ranked_geo_ids' array in JSON response."
                    )
                if "reasoning" not in parsed_data:
                    raise ValueError(
                        "Missing required 'reasoning' strings in JSON response."
                    )
                ...
                if not isinstance(ranked_ids, list):
                    raise ValueError(
                        "'ranked_geo_ids' structurally must be a strictly resolved list of Strings."
                    )
                ...
                raise RuntimeError(
                    f"Anthropic API execution framework failed during semantic matching operation: {e}"
                )

        raise ValueError(
            f"Failed to elicit valid target geos reliably from LLM mechanism after {max_retries} loop cycles. Last error trapped: {last_error}"
        )
