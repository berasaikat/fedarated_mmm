...
def run_incrementality_audit(
    ...
    if not treated_geo_id or not donor_geo_ids:
        raise ValueError(
            "The 'matched_geos' object strictly requires configured 'treated_geo_id' and 'donor_geo_ids' definitions explicitly."
        )

    mean_revenue = (
        geo_data[geo_data["geo_id"] == treated_geo_id]["revenue"].mean()
        if "geo_id" in geo_data.columns
        else geo_data["revenue"].mean()
    )
    ...
    inc_res = estimate_incrementality(
        pre_period_df, post_period_df, treated_geo_id, donor_weights
    )
    ...
    if not ch_summary:
        logger.warning(
            f"Target inference metric scalar '{channel_to_audit}' missing from global aggregate evaluations!"
        )
        mmm_beta_mean = 0.0
        mmm_beta_std = 0.0
    else:
